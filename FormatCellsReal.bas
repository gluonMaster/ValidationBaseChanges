...
Public Sub ConvertAndFormatCellsOptimized()
    ...
    ' --- 2) Convert column AY to 24-hour time format ---
'    Set rngAY = ws.Range("AY3:AY" & lastRow)
'    rngAY.Replace What:=" ", Replacement:="", LookAt:=xlPart
'
'    rngAY.TextToColumns Destination:=rngAY.Cells(1, 1), DataType:=xlDelimited, _
'                        TextQualifier:=xlDoubleQuote, ConsecutiveDelimiter:=False, _
'                        Tab:=False, Semicolon:=False, Comma:=False, Space:=False, _
'                        Other:=False
'
'    rngAY.NumberFormat = "hh:mm"
