VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} frmKinderNeueNH 
   Caption         =   "Liste der Neue Kinder fur Nachhilfe"
   ClientHeight    =   12384
   ClientLeft      =   108
   ClientTop       =   456
   ClientWidth     =   18900
   OleObjectBlob   =   "frmKinderNeueNH.frx":0000
   StartUpPosition =   1  'CenterOwner
End
Attribute VB_Name = "frmKinderNeueNH"
...
Dim arrNachHilfe(1 To 40) As String
...
'          Me.Controls("ComboBox" & i).RowSource = "[" & ThisWorkbook.name & "]PlusBText!F2:F" & iEnd
          Me.Controls("ComboBox" & i).List = arrNachHilfe
        Next i
   Else
...
'   aufsat = frmNeueElt.cboxAufsätze.Value
...
'   mitname = frmNeueElt.cboxName.Value
'   onename = frmNeueElt.cboxOneName.Value
   
   Me.Height = 350
End Sub

Sub arrNH(xFL As String, xiEnd As Integer)
  ...
  For i = 2 To xiEnd
    If InStr(Worksheets("PlusBText").Cells(i, xFL), "Nachhilfe") > 0 Or InStr(Worksheets("PlusBText").Cells(i, xFL), "NH ") > 0 _
        Or InStr(Worksheets("PlusBText").Cells(i, xFL), "Ind. ") > 0 Or InStr(Worksheets("PlusBText").Cells(i, xFL), "VSpE_") > 0 Then
      arrNachHilfe(j) = Worksheets("PlusBText").Cells(i, xFL)
      j = j + 1
    End If
  Next i
...
Private Sub btnOK_Click()
  ...
  For i = 1 To kindCount
    ...
    If nmon < 8 Then
        ...
        Worksheets("Kartei").Range("M" & (letzte + j)) = CDbl(Me.Controls("tbpreis" & i).text)
    ...
    Else
      ...
      Else
         Worksheets("Kartei").Cells((letzte + j), "T") = "NH"
      End If
    End If
...
'  Call SW_ElternHinzufuegen(SumSW_Monat, NeueEltName)
  ...
  Call AusdruckenNH(s_Pfad & "\vertrag Nachhilfe.doc")
...
Private Sub KorrekturNeueEltern(nrRowX As Integer)
  ...
  Workbooks("Korrektur.xlsx").Worksheets("NeueEltern").Cells(letzteRow, "N") = ThisWorkbook.Worksheets("Kartei").Cells(nrRowX, "N")
  Workbooks("Korrektur.xlsx").Worksheets("NeueEltern").Cells(letzteRow, "O") = ThisWorkbook.Worksheets("Kartei").Cells(nrRowX, "O")
  Workbooks("Korrektur.xlsx").Worksheets("NeueEltern").Cells(letzteRow, "P") = ThisWorkbook.Worksheets("Kartei").Cells(nrRowX, "P")
  Workbooks("Korrektur.xlsx").Worksheets("NeueEltern").Cells(letzteRow, "Q") = ThisWorkbook.Worksheets("Kartei").Cells(nrRowX, "Q")
  Workbooks("Korrektur.xlsx").Worksheets("NeueEltern").Cells(letzteRow, "R") = ThisWorkbook.Worksheets("Kartei").Cells(nrRowX, "U")
  ...
  Workbooks("Korrektur.xlsx").Worksheets("NeueEltern").Cells(letzteRow, 30) = "Nachhilfe"
...
Private Sub SW_ElternHinzufuegen(sumSW As Double, ElternName As String)
  ...
  j = 2
'  s_Pfad = ThisWorkbook.Path
  B = MappeOffen(s_Pfad & "\SW_Daten_24.xlsm")
  If B = False Then
       Workbooks.Open s_Pfad & "\SW_Daten_24.xlsm"
  Else
       Workbooks(s_Pfad & "\SW_Daten_24.xlsm").Activate
  End If
...
'Private Sub AusdruckenJ(pfad As String)
''    Dim DokumNm As String
'    Dim wda As Word.Application
'    Dim wdd As Word.Document
'
'    Set wdd = GetObject("C:\\Signatur\Unterrichtsvertrag 2022 neu.docx")
'    Set wda = wdd.Parent
'    wda.Visible = True
'
...
'
'
'Private Sub AusdrAnlage(pfad As String)
...
'    Dim wdd As Word.Document
'
'    Set wdd = GetObject("C:\\Signatur\Unterrichtsvertrag_Anlage.docx")
'    Set wda = wdd.Parent
'    wda.Visible = True
'
'    wdd.Bookmarks("VertNum").Select
...
'Private Sub AusdruckenM(pfad As String)
'    Dim DokumNm As String
'    Dim i As Integer
'    Dim KinderName As String
'    Dim wda As Word.Application
...
''    Dim KinderName As String
'
'    Set wdd = GetObject("C:\\Signatur\Unterrichtsvertrag 2022_Monatsvertrag.docx")
'    Set wda = wdd.Parent
'    wda.Visible = True
'
'    numExcel = Replace(numExcel, ". ", "")
...
Private Sub AusdruckenNH(pfad As String)
    Dim DokumNm As String
    Dim i As Integer
    Dim KinderName As String
    Dim wda As Word.Application
...
'    Dim KinderName As String
    ...
    Set wdd = GetObject("C:\\Signatur\vertrag Nachhilfe.doc")
    Set wda = wdd.Parent
    wda.Visible = True
 ...
 '*****************************************************
    For i = 1 To kindCount
        ...
        wdd.Bookmarks("Fach" & i).Select
        wda.Selection.TypeText (Me.Controls("ComboBox" & i).text)

        wdd.Bookmarks("StdWoch" & i).Select
        wda.Selection.TypeText (Me.Controls("tbtag" & i).text)
        
        wdd.Bookmarks("PreisSt" & i).Select
        wda.Selection.TypeText (Me.Controls("tbpreis" & i).text)

        wdd.Bookmarks("UnterZt" & i).Select
        wda.Selection.TypeText (Me.Controls("tbbem" & i).text)
    Next i
    ...
    wdd.SaveAs2 "C:\\Vertrage" & "\VertNH_1" & Right(numExcel, 4) & "_" & Left(NameElt, 6) & Left(vornameElt, 6) & ".pdf", _
    FileFormat:=wdExportFormatPDF
