Attribute VB_Name = "pay_DataProcessor"
'==========================
'   Module: pay_DataProcessor
'   Purpose: Data processing logic for family payment exports
'
'   Ported from Data-file/zahl_DataProcessor.bas to Superadmin
'   Changes:
'   - Made sheet-agnostic (accepts worksheet reference via pay_Main.wsKartei)
'   - All references to Worksheets("Kartei") replaced with pay_Main.wsKartei
'
'   Used by: pay_Main
'==========================
...
Public Sub ProcessAllData()
    ...
    ' Reads from pay_Main.wsKartei (set by caller)
    Dim i As Integer, j As Integer
    Dim cellValueA As Variant, cellValueB As Variant
    Dim ws As Worksheet
    
    Set ws = pay_Main.wsKartei
    
    ' Initialize first family with safe value retrieval
    cellValueA = ws.Cells(3, "A").Value
    cellValueB = ws.Cells(3, "B").Value
    
    pay_Main.num(1) = IIf(IsNull(cellValueA), "", CStr(cellValueA))
    pay_Main.nam(1) = IIf(IsNull(cellValueB), "", CStr(cellValueB))
    j = 1
    
    ' Process all rows from source worksheet
    For i = 4 To pay_Main.letzRow
        cellValueA = ws.Cells(i, "A").Value
        Dim currentNum As String
        currentNum = IIf(IsNull(cellValueA), "", CStr(cellValueA))
        
        If pay_Main.num(j) = currentNum Then
            ...
            If pay_Main.nam(j) <> currentName And Len(Trim(currentName)) > 0 Then
        ...
        Else
            ...
            cellValueB = ws.Cells(i, "B").Value
            pay_Main.num(j) = currentNum
            pay_Main.nam(j) = IIf(IsNull(cellValueB), "", CStr(cellValueB))
            Call ProcessSingleRecord(i, j)
        End If
    Next i
End Sub
...
Private Sub ProcessSingleRecord(row As Integer, familyIndex As Integer)
    ...
    cellValueJ = ws.Cells(row, "J").Value
    cellValueO = ws.Cells(row, "O").Value
    ...
    Select Case recordType
        ...
        Case 7 ' Conflict: Both Nachhilfe and Individual markers
            ...
            pay_Main.conflictCount = pay_Main.conflictCount + 1
...
' ============================================================
' COST ARRAY POPULATION - WRAPPERS
' ============================================================
...
' ============================================================
' COST ARRAY POPULATION - MAIN LOGIC
' ============================================================
...
Private Sub AddAmountsToArrays(row As Integer, familyIndex As Integer, arrayType As String, _
    ...
    ' Get amounts from Excel row (columns 21-32 = U-AF) with safe conversion
    For i = 1 To 12
        cellValue = ws.Cells(row, 20 + i).Value
        ...
        Case "individual"
            For i = 1 To 12
                ...
                Else
                    ...
                    If colorValue = 2 Then pay_Main.colorIndividual(familyIndex, i) = 2
                End If
            Next i
    End Select
End Sub
...
' ============================================================
' MONTH-SPECIFIC ARRAY UPDATES
' ============================================================

Private Sub AddToRegularMonth(familyIndex As Integer, month As Integer, amount As Double, colorValue As Integer)
    Select Case month
        Case 1: pay_Main.sum01(familyIndex) = pay_Main.sum01(familyIndex) + amount
        Case 2: pay_Main.sum02(familyIndex) = pay_Main.sum02(familyIndex) + amount
        Case 3: pay_Main.sum03(familyIndex) = pay_Main.sum03(familyIndex) + amount
        Case 4: pay_Main.sum04(familyIndex) = pay_Main.sum04(familyIndex) + amount
        Case 5: pay_Main.sum05(familyIndex) = pay_Main.sum05(familyIndex) + amount
        Case 6: pay_Main.sum06(familyIndex) = pay_Main.sum06(familyIndex) + amount
        Case 7: pay_Main.sum07(familyIndex) = pay_Main.sum07(familyIndex) + amount
        Case 8: pay_Main.sum08(familyIndex) = pay_Main.sum08(familyIndex) + amount
        Case 9: pay_Main.sum09(familyIndex) = pay_Main.sum09(familyIndex) + amount
        Case 10: pay_Main.sum10(familyIndex) = pay_Main.sum10(familyIndex) + amount
        Case 11: pay_Main.sum11(familyIndex) = pay_Main.sum11(familyIndex) + amount
        Case 12: pay_Main.sum12(familyIndex) = pay_Main.sum12(familyIndex) + amount
    End Select
    
    If colorValue > 0 Then pay_Main.colorRegular(familyIndex, month) = colorValue
    If HasNonZeroValues(familyIndex, "regular") Then pay_Main.nichtNull(familyIndex) = True
End Sub

Private Sub AddToNachhilfeMonth(familyIndex As Integer, month As Integer, amount As Double, colorValue As Integer)
    Select Case month
        Case 1: pay_Main.sum01N(familyIndex) = pay_Main.sum01N(familyIndex) + amount
        Case 2: pay_Main.sum02N(familyIndex) = pay_Main.sum02N(familyIndex) + amount
        Case 3: pay_Main.sum03N(familyIndex) = pay_Main.sum03N(familyIndex) + amount
        Case 4: pay_Main.sum04N(familyIndex) = pay_Main.sum04N(familyIndex) + amount
        Case 5: pay_Main.sum05N(familyIndex) = pay_Main.sum05N(familyIndex) + amount
        Case 6: pay_Main.sum06N(familyIndex) = pay_Main.sum06N(familyIndex) + amount
        Case 7: pay_Main.sum07N(familyIndex) = pay_Main.sum07N(familyIndex) + amount
        Case 8: pay_Main.sum08N(familyIndex) = pay_Main.sum08N(familyIndex) + amount
        Case 9: pay_Main.sum09N(familyIndex) = pay_Main.sum09N(familyIndex) + amount
        Case 10: pay_Main.sum10N(familyIndex) = pay_Main.sum10N(familyIndex) + amount
        Case 11: pay_Main.sum11N(familyIndex) = pay_Main.sum11N(familyIndex) + amount
        Case 12: pay_Main.sum12N(familyIndex) = pay_Main.sum12N(familyIndex) + amount
    End Select
    
    If colorValue > 0 Then pay_Main.colorNachhilfe(familyIndex, month) = colorValue
    If HasNonZeroValues(familyIndex, "nachhilfe") Then pay_Main.nichtNullN(familyIndex) = True
End Sub

Private Sub AddToIndividualMonth(familyIndex As Integer, month As Integer, amount As Double, colorValue As Integer)
    Select Case month
        Case 1: pay_Main.sum01I(familyIndex) = pay_Main.sum01I(familyIndex) + amount
        Case 2: pay_Main.sum02I(familyIndex) = pay_Main.sum02I(familyIndex) + amount
        Case 3: pay_Main.sum03I(familyIndex) = pay_Main.sum03I(familyIndex) + amount
        Case 4: pay_Main.sum04I(familyIndex) = pay_Main.sum04I(familyIndex) + amount
        Case 5: pay_Main.sum05I(familyIndex) = pay_Main.sum05I(familyIndex) + amount
        Case 6: pay_Main.sum06I(familyIndex) = pay_Main.sum06I(familyIndex) + amount
        Case 7: pay_Main.sum07I(familyIndex) = pay_Main.sum07I(familyIndex) + amount
        Case 8: pay_Main.sum08I(familyIndex) = pay_Main.sum08I(familyIndex) + amount
        Case 9: pay_Main.sum09I(familyIndex) = pay_Main.sum09I(familyIndex) + amount
        Case 10: pay_Main.sum10I(familyIndex) = pay_Main.sum10I(familyIndex) + amount
        Case 11: pay_Main.sum11I(familyIndex) = pay_Main.sum11I(familyIndex) + amount
        Case 12: pay_Main.sum12I(familyIndex) = pay_Main.sum12I(familyIndex) + amount
    End Select
    
    If colorValue > 0 Then pay_Main.colorIndividual(familyIndex, month) = colorValue
    If HasNonZeroValues(familyIndex, "individual") Then pay_Main.nichtNullI(familyIndex) = True
End Sub
...
Private Function HasNonZeroValues(familyIndex As Integer, arrayType As String) As Boolean
    ...
    Select Case LCase(arrayType)
        Case "regular"
            total = pay_Main.sum01(familyIndex) + pay_Main.sum02(familyIndex) + pay_Main.sum03(familyIndex) + _
                   pay_Main.sum04(familyIndex) + pay_Main.sum05(familyIndex) + pay_Main.sum06(familyIndex) + _
                   pay_Main.sum07(familyIndex) + pay_Main.sum08(familyIndex) + pay_Main.sum09(familyIndex) + _
                   pay_Main.sum10(familyIndex) + pay_Main.sum11(familyIndex) + pay_Main.sum12(familyIndex)
                   
        Case "nachhilfe"
            total = pay_Main.sum01N(familyIndex) + pay_Main.sum02N(familyIndex) + pay_Main.sum03N(familyIndex) + _
                   pay_Main.sum04N(familyIndex) + pay_Main.sum05N(familyIndex) + pay_Main.sum06N(familyIndex) + _
                   pay_Main.sum07N(familyIndex) + pay_Main.sum08N(familyIndex) + pay_Main.sum09N(familyIndex) + _
                   pay_Main.sum10N(familyIndex) + pay_Main.sum11N(familyIndex) + pay_Main.sum12N(familyIndex)
                   
        Case "individual"
            total = pay_Main.sum01I(familyIndex) + pay_Main.sum02I(familyIndex) + pay_Main.sum03I(familyIndex) + _
                   pay_Main.sum04I(familyIndex) + pay_Main.sum05I(familyIndex) + pay_Main.sum06I(familyIndex) + _
                   pay_Main.sum07I(familyIndex) + pay_Main.sum08I(familyIndex) + pay_Main.sum09I(familyIndex) + _
                   pay_Main.sum10I(familyIndex) + pay_Main.sum11I(familyIndex) + pay_Main.sum12I(familyIndex)
    End Select
