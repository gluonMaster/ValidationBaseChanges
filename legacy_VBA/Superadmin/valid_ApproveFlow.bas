...
Public Sub SyncDecisions()
    ...
    ' Current implementation: exactly one block per ID, so no conflicts expected.
    ' Dictionary serves as defensive aggregation in case of future multi-block scenarios.
    ...
    For r = 4 To lastRow Step 3 ' Process "Ist" rows only
        ...
        If decision = "APPROVED" Or decision = "DECLINED" Then
            ...
            If recordID <> "" Then
                ...
                ' (Should not happen with current implementation - one block per ID)
                If idDecisions.Exists(recordID) Then
                    ...
                    If CStr(existingData(1)) <> CStr(decisionData(1)) Then
                        ' Conflict: same ID has both Approved and Declined
                        On Error Resume Next
    ...
    ' Warn about conflicting decisions (same ID marked both Approved and Declined)
